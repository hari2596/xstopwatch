...
function Stopwatch() { // Define the Stopwatch component
  ...
  useEffect(() => { // Use the useEffect hook to manage side effects
    let interval; // Declare a variable to hold the interval ID

    if (isRunning) { // If the stopwatch is running
      interval = setInterval(() => { // Set up an interval to update the time every second
  ...
  const handleStartStop = () => { // Define a function to start or stop the stopwatch
    setIsRunning(!isRunning); // Toggle the isRunning state
  };

  const handleReset = () => { // Define a function to reset the stopwatch
    setTime(0); // Reset the time state
    setIsRunning(false); // Stop the stopwatch
  };

  const formatTime = (time) => { // Define a function to format the time
  ...
  return (
    <body // Render the body element with inline styles
      style={{
        display: "flex",
    ...
    >
      <div // Render a container div with inline styles
        style={{
          display: "flex",
          justifyContent: "center",
      ...
      >
        ...
        <button // Render the start/stop button with inline styles
          style={{
            padding: "8px 16px",
            fontSize: "16px",
            marginBottom: "10px",
          }}
          onClick={handleStartStop} // Call handleStartStop when the button is clicked
        >
          {isRunning ? "stop" : "start"} 
        </button>

        <button // Render the reset button with inline styles
          style={{ padding: "8px 16px", fontSize: "16px" }}
          onClick={handleReset} // Call handleReset when the button is clicked
        >
          Reset 
        </button>
      </div>
